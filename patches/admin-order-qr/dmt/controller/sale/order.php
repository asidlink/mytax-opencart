...
class Order extends \Opencart\System\Engine\Controller {
	...
	public function info(): void {
		...
		// ApiShip: вкладка «Доставка (СДЭК)» на странице заказа.
		// Раньше её добавлял ocmod apiship_oc41_compat, но в этой сборке OpenCart 4.1
		// ocmod-модификации не применяются, поэтому вкладка подключается напрямую.
		if (!empty($data['shipping_method_code']) && strpos($data['shipping_method_code'], 'apiship') !== false && !empty($order_info)) {
			$this->load->language('extension/apiship/shipping/apiship');
			$output = $this->load->controller('extension/apiship/shipping/apiship.order');
		...
		if (!empty($order_info['order_id'])) {
			...
			if ($receipt && $receipt['status'] === 'completed' && !empty($receipt['qr_code_path'])) {
				$base = defined('HTTP_CATALOG') ? HTTP_CATALOG : 'https://xn--80aanved7b4e.xn--p1ai/';
	...
	public function call(): void {
		...
		if (!$json) {
			...
			// Полный список ПВЗ (не только «сводный» вариант) для выбора
			// конкретного пункта выдачи в админке при смене доставки.
			$store->session->data['api_id'] = 1;
