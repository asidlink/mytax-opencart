...
// Admin model - идемпотентная установка событий
$adminModel = <<<'PHP'
<?php
...
class Mytax extends \Opencart\System\Engine\Model {
    public function install(): void {
        ...
        $this->model_setting_event->addEvent(["code" => "mytax_checkout_success", "description" => "Создание чека на странице успеха", "trigger" => "catalog/view/checkout/success/before", "action" => "extension/mytax/module/mytax.viewSuccess", "status" => 1, "sort_order" => 1]);
    }
...
// Catalog model - защита от повторного чека (без изменений, как в v2.5)
$catModel = <<<'PHP'
<?php
...
// Catalog controller - КЛЮЧЕВОЕ: проверяем, что заказ ОПЛАЧЕН (order_status_id > 0)
$catCtrl = <<<'PHP'
<?php
...
class Mytax extends \Opencart\System\Engine\Controller {
    ...
    public function orderHistory(string &$route, array &$args): void {
        ...
        $this->load->model("checkout/order");
        $order_info = $this->model_checkout_order->getOrder($order_id);
        // Чек создаём ТОЛЬКО для оплаченных заказов (order_status_id > 0)
    ...
    // Страница успеха: чек ТОЛЬКО если заказ реально оплачен Ю-Kасой
    public function viewSuccess(string &$route, array &$args): void {
        ...
        if ($order_id) {
            $this->load->model("checkout/order");
            $order_info = $this->model_checkout_order->getOrder($order_id);
            // ВАЖНО: проверяем, что заказ оплачен (order_status_id > 0).
            // Если Ю-Касса не подтвердила оплату (status=0), чек НЕ создаём!
            if ($order_info && $order_info["order_status_id"] > 0) {
                $this->model_extension_mytax_checkout_mytax->createReceipt($order_id, $order_info["email"]);
            }
        }
