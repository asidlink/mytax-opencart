...
// Admin model - без изменений (идемпотентные события)
$adminModel = <<<'PHP'
<?php
...
// Catalog model - ИСПРАВЛЕН: время в UTC! (была ошибка часового пояса)
$catModel = <<<'PHP'
<?php
    ...
    public function createReceipt(int $order_id, string $email): array {
        ...
        for ($attempt = 1; $attempt <= $maxRetries; $attempt++) {
            try {
                ...
                // ==== ИСПРАВЛЕНО: время в UTC (не локальное!) ====
...
// Catalog controller - без изменений (чек в письме)
$catCtrl = <<<'PHP'
<?php
...
echo "ГОТОВО: $target (v3.0.1 — время UTC исправлено!)\n";
$z2 = new ZipArchive();
$z2->open($target);
