...
// Admin model - удаляет дубли событий
$adminModel = <<<'PHP'
<?php
...
// Catalog controller - исправляем URL через config_url (надёжнее чем HTTP_CATALOG)
$catCtrl = <<<'PHP'
<?php
...
class Mytax extends \Opencart\System\Engine\Controller {
    ...
    public function viewOrderHistory(string &$route, array &$args): void {
        ...
        if ($order_id) {
            ...
            if ($r) {
                // Полный URL через config_url (всегда доступен в админке и каталоге)
